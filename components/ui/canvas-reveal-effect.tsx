...
const ShaderMaterial = ({
  ...
  const getUniforms = useCallback(() => {
    ...
    preparedUniforms["u_resolution"] = {
      ...
      type: "2f", // Added the missing 'type' property
  ...
  const materialObject = useMemo(() => {
    const materialObject: THREE.ShaderMaterial = new THREE.ShaderMaterial({
      uniforms: getUniforms(),
      fragmentShader: source,
    });
    return materialObject;
  }, [source, getUniforms, size.width, size.height]);

  return (
    <mesh ref={ref}>
      <planeGeometry args={[size.width * 2, size.height * 2, 1, 1]} />
      <primitive attach="material" object={materialObject} />
    </mesh>
  );
};
